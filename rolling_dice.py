...
def parse_input():
    ...
    """
    while True:
        input_string = input("Enter a number from 1 to 6: ").strip()  # Ask for input
        
        # Check if the input is a valid number between 1 and 6
        if input_string in {"1", "2", "3", "4", "5", "6"}:
            return int(input_string)  # Return the valid input as an integer, exiting the loop
        else:
            print("Please enter a number from 1 to 6.")  # Print the error message
...
def main():
    while True:
        ...
        # 5. Ask if the user wants to roll again
        user_choice = input("Do you want to roll the dice again? (y/n): ").strip().lower()
